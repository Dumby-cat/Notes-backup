...
$$
$$
y(t) = \int_{-\infty}^{+\infty}h(\tau)x(t - \tau)\mathrm{d}\tau = \int_{-\infty}^{+\infty}h(\tau)\mathrm{e}^{s(\tau -\tau)}\mathrm{d}\tau
$$
$$
y(t) = \mathrm{e}^{s t}\int_{-\infty}^{+\infty}h\left(\tau\right)\mathrm{e}^{-s\tau}\mathrm{d}\tau = H(s)e
$$
其中：
$$
H(s) = \int_{-\infty}^{+\infty}h(\tau)\mathrm{e}^{-s\tau}\mathrm{d}\tau
$$
上式称为**系统函数**，$e^{st}$ 称为 LTI 系统的**特征函数**（类比线代中的特征向量），对某一给定的 $\pmb{s}$ 值，常数 $H(s)$ 就是与特征函数 $\mathbf{e}^{\mathbf{z}}$ 有关的特征值。
...
$$
易知 $x(t)$ 基础周期为 $T$。
上式称为周期信号的**傅里叶级数**（Fourier Series）表示的**综合（synthesis）公式**。

$x(t)$ 为一实信号，有 $x^*(t)=x(t)$，其中 $x^*(t)$ 表示耦合。
$$
...
则 $a_{-k}e^{-jk\omega t}$ 和 $a_{k}e^{jk\omega t}$ 共轭，$a_{-k}e^{-jk\omega t}＋a_{k}e^{jk\omega t}＝\Re\{a_ke^{jk\omega t}\}$。
所以有
$$
x(t) = \sum_{k = -\infty}^{+\infty}a_{k}\mathrm{e}^{\mathrm{j}k\omega_{0}t} ＝a_{0} + \sum_{k = 1}^{\infty}2\mathcal{R}_{k}\{a_{k}\mathrm{e}^{\mathrm{i}k\omega_{0}t}\}
$$
令 $a_{k} = B_{k} + \mathrm{j}C_{k}$，有
$$
x(t) = a_{0} + 2\sum_{k = 1}^{\infty}\left[B_{k}\cos k\omega_{0}t - C_{k}\sin k\omega_{0}t\right]
$$
同样为 FS 表示。
...
$$
$$
\int_{0}^{T}x(t)\mathrm{e}^{-\mathrm{i}n\omega_{0}t}\mathrm{d}t = \sum_{k = -\infty}^{+\infty}a_{k}\bigg[\int_{0}^{T}\mathrm{e}^{\mathrm{i}(k - n)\omega_{0}t}\mathrm{d}t\bigg]
$$
$$
\int_{0}^{T}\mathrm{e}^{\mathrm{i}(k - n)\omega_{0}t}\mathrm{d}t = \int_{0}^{T}\cos ((k - n)\omega_{0}t)\mathrm{d}t + \mathrm{j}\int_{0}^{T}\sin ((k - n)\omega_{0}t)\mathrm{d}t
$$
得到
$$
\int_{0}^{T}\mathrm{e}^{\mathrm{i}(k - n)\omega_{0}t}\mathrm{d}t = \left\{ \begin{array}{l l}{T,} & {k = n}\\ {0,} & {k\neq n} \end{array} \right.
$$
故得到**傅里叶系数（频谱系数）**（Fourier series coefficients(spectral coefficients)）：
$$
a_{n} = \frac{1}{T}\int_{0}^{T}x(t)\mathrm{e}^{-\mathrm{i}n\omega_{0}t}\mathrm{d}t
$$
上式称为**分析公式**。
