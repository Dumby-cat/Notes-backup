...
$$
$$
\int_{-\infty}^{+\infty}e^{\pm j\omega t}\mathrm{d}t=2\pi\delta(\omega)
$$
$$
P_{2T_1}(t)\xleftrightarrow{FT}\frac{\sin(\omega T_{1})}{\omega /2} = 2T_1\frac{\sin(\omega T_{1})}{\omega T_1}
$$
$$
sgn(t)=\begin{cases}1,\quad x>0;\\-1,\quad x<0.\end{cases}\quad  sgn(t)\overset{FT}{\leftrightarrow}\frac{2}{\mathrm{j}\omega}
$$
$$
...
$$
#### 对偶性
对于两个信号 $a(t)\leftrightarrow A(j\omega),\quad b(t)\leftrightarrow B(j\omega)$，若 $a(t)=B(\omega)$，则 $A(\omega)=2\pi b(-\omega)$。也就是说 $f(t)\leftrightarrow F(\mathrm{j}\omega) \Longrightarrow \frac{1}{2\pi} F(-\mathrm{j}t)\leftrightarrow f(\omega)$。
例如
$$
...
$$
$$
x_{p}(t) = \sum_{n = -\infty}^{+\infty}x(n T)\delta (t - n T),\quad X_{p}(\mathrm{j}\omega) = \frac{1}{2\pi} [ X(\mathrm{j}\omega) * P(\mathrm{j}\omega) ] \tag{7.3}
$$
$$
P(\mathrm{j}\omega) = \frac{2\pi}{T}\sum_{k = -\infty}^{+\infty}\delta (\omega - k\omega_{s}),\quad X_{p}(\mathrm{j}\omega) = \frac{1}{T}\sum_{k = -\infty}^{+\infty}X(\mathrm{j}(\omega - k\omega_{s})) \tag{7.5}
$$
...
线性内插：
	将相邻的样本点用直线直接连起来。
带限内插：
	利用理想低通滤波器的单位冲激响应的内插（通过sinc函数的叠加还原信号）。

### 欠采样的效果：混叠现象（aliasing）
> 混叠现象是指对连续时间信号进行离散采样时，因采样频率低于信号最高频率的两倍（即不满足奈奎斯特定理），导致高频分量被错误映射到低频区域，使重建信号出现虚假低频成分的失真现象。这时 $X_{p}(\mathrm{j}\omega) = \frac{1}{T}\sum_{k = -\infty}^{+\infty}X(\mathrm{j}(\omega - k\omega_{s}))$ 中的各个单项发生重叠。

当混叠现象发生时,原始频率 $\omega_{0}$ 就被混叠成一个较低的频率 $\left(\omega_{s} - \omega_{0}\right)$。采样定理明确要求采样频率**大于**信号中最高频率的两倍，而不是大于或等于最高频率的两倍，等于时可能会出现相位倒置。
...
## 第八章：通信系统
### 复指数与正弦幅度调制
一个复指数信号或正弦信号 $c(t)$ 的振幅被载有信息的信号 $x(t)$ 相乘（或调制）。信号  $x(t)$  一般称为调制信号，而信号 $c(t)$ 称为载波信号，已调信号  $y(t)$  就是这两个信号的乘积，即 $y(t) = x(t)c(t)$。
...
则有
$$
Y(\mathrm{j}\omega) = \frac{1}{2\pi} X(\mathrm{j}\omega)*C(\mathrm{j}\omega) = X(\mathrm{j}\omega -\mathrm{j}\omega_{c}) \tag{8.4}
$$
$$
y=x(t)c(t)=x(t)e^{jw}=x(t)\cos\omega_{c}t + \mathrm{j}x(t)\sin\omega_{c}t
$$

### 正弦幅度调制的解调
#### 同步解调
![[Pasted image 20251219181052.jpg]]
假设解调器载波在相位上与调制器载波是同相的，这一过程称为同步解调。不同步时恢复出来的 $x(t)$ 带有一个振幅因子。
$$
w(t) = x(t)cos(\omega_{c} t + \theta_{c})cos(\omega_{c} t + \phi_{c}) \tag{8.17}
$$
$$
\cos (\omega_{c}t + \theta_{c})\cos (\omega_{c}t + \phi_{c}) = \frac{1}{2}\cos (\theta_{c} - \phi_{c}) + \frac{1}{2}\cos (2\omega_{c}t + \theta_{c} + \phi_{c}) \tag{8.18}
$$
得到
$$
\omega (t) = \frac{1}{2}\cos (\theta_{c} - \phi_{c})x(t) + \frac{1}{2} x(t)\cos (2\omega_{c}t + \theta_{c} + \phi_{c}) \tag{8.19}
$$
这样，低通滤波器的输出就是  $x(t)$  乘以振幅因子  $\cos (\theta_{c} - \phi_{c})$。

#### 非同步解调
两个基本假设：
1. $x(t)$ 总是正的；
2. $x(t)$ 变化比 $\omega_c$ 慢的多。

令  $K$  是  $x(t)$  的最大幅度值，$K / A$  之比称为调制指数  $m$ ; 若以百分数表示, 则称为调制百分数。
...
4. 如果  $x(t)$  是右边信号，**而且如果  $\mathcal{R}\{s\} = \sigma_{0}$  这条线位于ROC内**，那么  $\mathcal{R}_{0}\{s\} >\sigma_{0}$  的全部  $\pmb{s}$  值都一定在ROC内；
5. 如果  $x\left(t\right)$  是左边信号，**而且如果  $\mathcal{R}\left\{s\right\} = \sigma_{0}$  这条线位于ROC内**，那么  $\mathcal{R}_{\mathfrak{e}}\left\{s\right\} < \sigma_{0}$  的全部  $\pmb{s}$  值也一定在ROC内；
6. 如果  $x(t)$  是双边信号，**而且如果  $\mathcal{R}\left\{s\right\} = \sigma_{0}$  这条线位于ROC内**，那么ROC就一定是由s平面的一条带状区域所组成，直线  $\mathcal{R}_{0}\left\{s\right\} = \sigma_{0}$  位于带中；
...
$$
\mathcal{X}(s) \triangleq \int_{0^{-}}^{\infty} x(t) \mathrm{e}^{-s t} \mathrm{d} t \tag{9.170}
$$
记作 $x(t)\xleftrightarrow{\mathcal{UL}}\mathcal{X}(s)=\mathcal{UL}\{x(t)\}$。
单边拉普拉斯变换可以看作将 $x(t)$ 变为 $x(t)u(t)$ 的双边拉普拉斯变换。
#### 单边拉普拉斯变换的性质
![[Pasted image 20260307140724.jpg]]
常用的单边拉普拉斯变换对大部分将双边的信号的 $u(t)$ 变成 1 即可，注意 $\delta(t)$ 仍然是 $\delta(t)$，$u(t)$ 本身也可以是其本身。

| 时域函数 $f(t) (t \ge 0)$     | 复频域函数 F(s)                          | 收敛域 (ROC)    |
| ------------------------- | ----------------------------------- | ------------ |
| $\delta(t)$               | 1                                   | 全 s 平面       |
| $u(t) 或 1$                | $\frac{1}{s}$                       | $Re(s) > 0$  |
| $t$                       | $\frac{1}{s^2}$                     | $Re(s) > 0$  |
| $t^n (n = 1, 2, \dots)$   | $\frac{n!}{s^{n+1}}$                | $Re(s) > 0$  |
| $e^{-at}$                 | $\frac{1}{s+a}$                     | $Re(s) > -a$ |
| $t e^{-at}$               | $\frac{1}{(s+a)^2}$                 | $Re(s) > -a$ |
| $\frac{t^{n}e^{-at}}{n!}$ | $\frac{1}{(s+a)^{n+1}}$             | $Re(s) > -a$ |
...
#### 重要例子
$$
a^{n}u[n]\xleftrightarrow{Z} \frac{1}{1-az^{-1}},\quad |z|>|a|
$$
