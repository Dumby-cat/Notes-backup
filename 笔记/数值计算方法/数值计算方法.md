...
$x^*$ 为 $g$ 的不动点，$|g'(x_0)|\leq L<1$，则 $g$ 部分收敛。

#### 迭代收敛的加速方法
构造 $(1+\theta)x=g(x)+\theta x$，得到 $x=\frac{g(x)+\theta x}{1+\theta}$。选取特殊的 $\theta$ 可以加速迭代收敛。

### 斯蒂芬森加速迭代法
从初值 $x_0$ 出发，计算 $\overline{x_1}=g(x_0),\overline{x_2}=g(x_1)$，在曲线 $y=g(x)$ 上得到两个点 $P_0(x_0,\overline{x_1}),P_1(\overline{x_1},\overline{x_2})$，用直线连接两点，它们与 $y=x$ 的交点设为 $P_3$，则 $P_3$ 坐标为 $(\hat{x},\hat{x})$，有
$$
\frac{\hat{x}-\overline{x_1}}{\hat{x}-x_0}=\frac{\overline{x_2}-\overline{x_1}}{\overline{x_1}-x_0}
$$
得到
$$
\hat{x}=\frac{x_0\overline{x_2}-\overline{x_1}^2}{x_0-2\overline{x_1}+\overline{x_2}}
$$

### 牛顿法
作  $f(x)$  在  $x_{0}$  的Taylor展开:
$$
f(x) = f(x_{0}) + f^{\prime}(x_{0})(x - x_{0}) + \frac{f^{\prime\prime}(x_{0})}{2!} (x - x_{0})^{2} + \dots
$$
取展开式的线性部分作为  $f(x)\approx 0$  的近似值,则有 $f(x_{0}) + f^{\prime}(x_{0})(x - x_{0})\approx 0$，令 $x_{1} = x_{0} - \frac{f(x_{0})}{f^{\prime}(x_{0})}$，再作  $f(x)$  在  $x_{1}$  的Taylor展开并取其线性部分得到 $x_{2} = x_{1} - \frac{f(x_{1})}{f^{\prime}(x_{1})}$。
一直做下去得到Newton迭代格式:
$$
x_{k + 1} = x_{k} - \frac{f(x_{k})}{f^{\prime}(x_{k})},\quad k = 1,2,\dots
$$
Newton 迭代对应于  $f(x) = 0$  的迭代方程是  $\phi (x) = x - \frac{f(x)}{f^{\prime}(x)}$
$$
\phi^{\prime}(x) = \frac{f(x)f^{\prime\prime}(x)}{(f^{\prime}(x))^{2}}
$$
若  $\alpha$  是  $f(x)$  的单根时，则有  $|\phi^{\prime}(\alpha)| = 0$  ,只要初值  $x_{0}$  充分接近  $\alpha$ $|\phi^{\prime}(x_{0})|< 1$  ,Newton迭代则收敛。
若存在  $M > 0$
$$
\lim_{k\to \infty}{\frac{|e_{k + 1}|}{|e_{k}|^{n}}} = \lim_{k\to \infty}{\frac{|x_{k + 1} - \alpha|}{|x_{k} - \alpha|^{n}}} = M
$$
称迭代格式收敛的阶为  $n$ 。

### 割线法（弦截法）
在Newton迭代格式中:  $x_{k + 1} = x_{k} - \frac{f(x_{k})}{f^{\prime}(x_{k})}$ 。
用差商  $f[x_{k - 1}, x_{k}] = \frac{f(x_{k}) - f(x_{k - 1})}{x_{k} - x_{k - 1}}$  代替导数  $f^{\prime}(x_{k})$ , 并给定两个初始值  $x_{0}$ 和  $x_{1}$ , 得到弦截法迭代格式:
$$
x_{k + 1} = x_{k} - \frac{f(x_{k})(x_{k} - x_{k - 1})}{f(x_{k}) - f(x_{k - 1})}, \quad k = 1,2, \dots
$$
...
### 迭代法的基本概念
*向量序列的极限*：
	定义了范数 $\boldsymbol{\|\cdot\|}$ 的向量空间 $\boldsymbol{R^n}$，如果 $\boldsymbol{\exists x \in R^n}$ 使得 $\boldsymbol{\lim\limits_{k \to \infty} \|x^{(k)} - x\| = 0}$，则称 $\boldsymbol{\{x^{(k)}\}}$ 收敛于 $\boldsymbol{x}$ 记为 $\boldsymbol{\lim\limits_{k \to \infty} x^{(k)} = x}$

简单来说就是 k 趋于无穷时每一位的值组成向量。
...
#### 迭代法的一般公式
$$
x^{(k+1)}=F_{k}\{x^{(k)},x^{(k-1)},\dots,x^{(k-m)}\},k=0,1,\dots
$$
前 $k+1$ 步与前 $m+1$ 步有关，成为多步迭代法。$m=0$ 称为单步迭代法。$F_k$ 是线性的则称为单步线性迭代法，$x^{(k+1)}=B_{k}x^{(k)}+f_{k},k=0,1,\dots$。$B_{k},f_{k}$ 与 k 无关，称之为单步定常线性迭代法。


### Jacobi （雅克比）和 Gauss-Seidel （高斯希德尔）迭代法
#### 雅克比迭代法
设方程组 $\boldsymbol{Ax = b}; \boldsymbol{A = (a_{ij})_{n\times n}}, \boldsymbol{b = (b_{i})_{1\times n}}; \boldsymbol{\det(A)\neq 0}$。
将系数矩阵分裂为: $\boldsymbol{A = D - L - U}$，其中$\boldsymbol{D = \text{diag}(a_{11}, a_{22}, \dots, a_{nn})}$；
...
如果 $a_{ii}\neq0(i = 1,2,\cdots,n)$，
原方程组可化为 $x = D^{-1}(L + U)x+D^{-1}b=Bx + f$，
其中 $B = D^{-1}(L + U)=(I - D^{-1}A);f = D^{-1}b$，
相应的迭代格式 $x^{(k + 1)}=Bx^{(k)}+f;k = 0,1,2,\cdots$。
上述方法称为Jacobi迭代法，简称J法或简单迭代法

分量形式：
$$
x_{i}^{(k + 1)}=\frac{b_{i}-\sum_{j = 1}^{i - 1}a_{ij}x_{j}^{(k)}-\sum_{j = i + 1}^{n}a_{ij}x_{j}^{(k)}}{a_{ii}};i = 1,2,\cdots,n
$$